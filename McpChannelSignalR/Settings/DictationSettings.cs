...
public record DictationSettings
{
    ...
    // header. A duration cannot be enforced on a blob without decoding it, and the point of the
    // check is to refuse the oversized post before anything decodes anything.
    public long MaxBytes => 44 + (long)(MaxLength.TotalSeconds * 16_000 * 2);
}