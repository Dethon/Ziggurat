...
- [x] A scenario declares "A before B" pairs and fails when B precedes A.
- [x] Unrelated calls interleaved between a constrained pair do not fail the scenario.
- [x] A pair whose second call never happens fails, distinguishably from an out-of-order pair.
- [x] The extend-a-running-timer scenario passes with its three-step order declared.
- [x] Ordering checks are proven to fail correctly against a scripted chat client.
