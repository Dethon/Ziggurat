...
- [x] Three endpoints on the Agent, which already hosts an HTTP API and already registers custom
...
- [x] One shared bearer secret gates all three. It is a secret, so it gets a placeholder in the
      compose env file wired through as a variable, never a real value.
- [x] One Redis key per outpost registration, refreshed by the keepalive, with a ninety second
      expiry against a thirty second refresh interval.
- [x] The registration's name is its identity and the last write wins, so a machine that restarts
      re-registers over its own entry with no special handling.
- [x] Taking a registration back removes it immediately rather than waiting for expiry.
- [x] A metric is published when a registration lands, is refreshed and expires, so the question
      "was that machine up at two o'clock" is answerable.
- [x] Unit tests drive the registry type over an injected store and a controllable clock: land,
...
- [x] One integration test on the shared Redis fixture proves the key really disappears. The
