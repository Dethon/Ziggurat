...
- [ ] Image fetch calls `OnRefAsync` per ref and performs no session bookkeeping of its own; the last old-verb call sites in the browser are gone
- [ ] One closed tab walls only its own images; other refs in the same request still return bytes
- [ ] Ordering fact: a fetch whose tab closed mid-work answers the closed wall even when the work reported a refusal (red first — replaces the third disambiguation spelling)
- [ ] Image stamping uses the image namespace's counters and lease through the method's stamping policy
- [ ] Existing suites stay green; nothing in the Playwright browser sequences the tab protocol any more
