...
- [ ] A recording with no load call against a scenario that requires one yields a failure whose kind says the skill was not loaded; the same recording with the load present and a behaviour check failing yields the other kind.
- [ ] A load outside a scenario's permitted set is an unnecessary call, as any other tool is.
- [ ] The scorecard's per-scenario and per-claim rows carry the failure kind, and the dump shows it beside the recording.
- [ ] Spec: `.scratch/prompt-skills/spec.md` § Eval.
