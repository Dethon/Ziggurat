...
- [ ] `McpAgentSkillsTests` shows the load tool's description is the repo's and its schema lists exactly the session's skill names as an enum; a load with a listed name still returns the body; the resource and script tools are still absent.
- [ ] The prompt snapshots are unchanged (the tool's face is not in the instructions).
- [ ] One full-tier armed pass (`ZIGGURAT_EVAL=1 dotnet test --filter "Category=Eval&Tier=Full"`, launched with `setsid nohup` and a log — the Bash tool's ten-minute cap kills a backgrounded run; never rebuild while it runs). Back up `.eval-output/scorecard-full.json` first. Count the `unnecessary call: load_skill` lines across `.eval-output/*.md` and `.eval-output/flakes/*.md` newer than the backup, and note the count in the commit against 7 of 228.
- [ ] Spec: `.scratch/prompt-skills/spec.md` § Transport and ownership; the decision on the rate is in `handoff.md`.
