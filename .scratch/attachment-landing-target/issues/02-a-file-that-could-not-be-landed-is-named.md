...
- [x] A file whose write fails is named to the model on the way out, through the same step that
      already names the files that landed.
- [x] An exec-capable mount that declares no workspace lands nothing and the model is told which
      files could not be placed.
- [x] A partly landed message names only the files that failed and still reports the ones that
      landed.
- [x] The failed names are recorded on the message alongside the landed paths, so the turn's
      record is complete.
- [x] The failure notice is spoken within the hydration distance and silent past it: beyond that
      distance the model has neither the bytes nor the file, and a notice about neither is noise.
- [x] Landed paths keep their unbounded life, because the file is still there.
- [x] Asserted at the hydration depth seam, beside the existing cases for an attachment within
