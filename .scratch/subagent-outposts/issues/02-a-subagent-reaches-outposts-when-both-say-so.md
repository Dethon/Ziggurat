...
- [ ] An outposts opt-in on the subagent definition, named as it is on an agent definition,
...
- [ ] The shipped worker profile turns the opt-in on. Both agents that enable the subagents feature
      are already opted in, so this is live for chat and for voice on one edit.
- [ ] Unit tests at the projection: both true yields an opted-in spec; parent true and definition
      false, parent false and definition true, and both false all yield an opted-out one.
- [ ] A settings-binding test that the worker profile's opt-in is bound from configuration and that
      an omitted one is false.
- [ ] An integration test that a subagent spawned from an opted-in parent has a live outpost among
      its mounts, driven through the existing outpost mounting fixture.
- [ ] No change to endpoint composition, the dial policy or filesystem discovery. A subagent's
...
- [ ] Red first, in that order: the projection tests fail on the hardcoded false before the rule
