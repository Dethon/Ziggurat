...
- [x] A spawn context record in the agent domain namespace, beside the agent key and the disposable
...
- [x] The agent factory contract's subagent entry point takes the definition, the approval handler
      and that record, replacing the four positional parameters.
- [x] The multi-agent factory composes the record from the spec it is already building against, so
      the parent's values reach the projection with no new lookup and no second source of truth.
- [x] The projection's subagent entry point reads the conversation, user and whitelist patterns off
      the record. The opt-in stays unread; the hardcoded false it will replace is untouched here.
- [x] The fake agent factory used by the integration fixtures follows the contract.
- [x] The subagent tool and its feature config are unchanged. The spawn delegate stays "a definition
      in, a running agent out" — the parent's contribution is captured, as the conversation and
      whitelist already are.
- [x] Existing projection and subagent tests pass unmodified except where they construct the new
