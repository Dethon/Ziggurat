...
- [x] The agent spec carries whether this build records mount verdicts: true from the agent entry
      point, false from the subagent one. A field with a value, like every other difference between
      an agent and a subagent.
- [x] The build step reads that field where verdicts are recorded and never asks which kind of agent
      it is building.
- [x] Endpoint composition is unchanged. A subagent still receives the names of the outposts it was
      given; it simply writes nothing back.
- [x] Unit tests at the projection that the flag is true for an agent and false for a subagent.
- [x] A test that a subagent's session build leaves an existing registration's verdict as it was,
      driven through the seam that records them rather than by reaching into the registry's storage.
- [x] Nothing is reused for this — the existing history flag is not overloaded to mean two things.
