...
**A cross-origin picture is read off a canvas, gated by CORS all the same.** Same-origin uses `fetch` with the page's credentials and keeps the bytes exactly as served — but only for a raster the chat wire accepts (png/jpeg/gif/webp); anything else falls through to the canvas and leaves as PNG, because as-served `image/svg+xml` (Wikipedia's own site logo, the one image its pages serve same-origin) made the vision provider refuse the whole request with HTTP 400. Cross-origin loads the image anonymously (`crossOrigin='anonymous'`) and reads it back off a canvas, which re-encodes it as PNG. That anonymous load needs the CDN's `Access-Control-Allow-Origin` exactly as an anonymous `fetch` would — the big image CDNs send `*`, and a wildcard is precisely what a **credentialed** request is rejected against, which is what broke the first implementation. Don't "fix" this back to `credentials: 'include'`. A CDN sending no ACAO at all — JPL's gallery CloudFront, live — fails the anonymous probe and the fallback to the rendered element taints the canvas; a tainted canvas is not a refusal but two more rungs: **re-request the address through the context's request client** (CORS binds script inside the page and nothing else; same cookies, bytes as served, wire rasters only), then a **Playwright element screenshot** of the pixels the compositor already painted (rendered box as PNG) (`docs/adr/0033`, amended). SiteRefused remains only for an image the browser itself will not show.
