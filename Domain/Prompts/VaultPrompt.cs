...
public static class VaultPrompt
{
    ...
    public const string Prompt = """
        ...
        - **Headings are referenceable.** Other notes may link to `[[ThisNote#Some Heading]]`. Renaming a heading breaks those links — search for incoming `[[...#...]]` references and update them in the same change, exactly as a filename rename updates its wikilinks.
        ...
        - Writes are restricted to a configured set of text extensions; attempts outside that set return an error envelope. The error tells you which extensions are accepted — pick the one that fits and write the file yourself, then mention the substitution in your reply. The extension is a mount constraint, not a decision to hand back as a question.
