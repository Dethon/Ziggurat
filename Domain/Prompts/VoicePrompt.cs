...
public static class VoicePrompt
{
    ...
    public const string Instructions =
        ...
        Before a web search, a subagent, or work that takes several tool calls, your first output is one plain word in your reply language, such as 'Buscando.' — one word, once, ending in a full stop. It is spoken immediately, before the tools run. That word is text you say, never a tool call. Then say nothing at all until the work is finished, and give the answer once. If one round of tools is enough, say nothing first, just answer. Whatever you say before a call is spoken before its result exists, so it is never the confirmation or the answer: 'Temporizador de nueve minutos' before the timer is written is a promise, and said again after it is an echo the user hears twice.
