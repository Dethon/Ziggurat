...
public class MemoryJudgeVerifyTests
{
    ...
    private static readonly IReadOnlyList<ChatMessage> Window = [new(ChatRole.User, "¿qué tiempo hace el martes?")];
    ...
    public async Task ACandidateBelowTheBarOnAnyOneQuestion_IsDropped_AndTheDropCarriesContentAndScores()
    {
        ...
        var verdict = await Judge(judge, published).VerifyAsync(Window, Candidate("Preguntó por el tiempo el martes"), Context, CancellationToken.None);
    ...
    public async Task ACandidateAtTheBarOnAllFour_IsStored()
    {
        ...
        var verdict = await Judge(judge, published).VerifyAsync(Window, Candidate("Se levanta a las siete"), Context, CancellationToken.None);
    ...
    public async Task AnInstruction_IsJudgedOnSupportedAlone()
    {
        ...
        var verdict = await Judge(judge).VerifyAsync(
            Window, Candidate("Instrucción: tratarle siempre de tú", MemoryCategory.Instruction), Context, CancellationToken.None);
    ...
    public async Task AnInstructionNobodyStated_IsStillDropped()
    {
        ...
        var verdict = await Judge(judge).VerifyAsync(
            Window, Candidate("Instrucción: no poner música", MemoryCategory.Instruction), Context, CancellationToken.None);
    ...
    {
        var verdict = await Judge(StubJudge.Absent(reason)).VerifyAsync(Window, Candidate("anything"), Context, CancellationToken.None);
    ...
    public async Task AQuestionLeftUnanswered_DoesNotDrop()
    {
        ...
        var verdict = await Judge(judge).VerifyAsync(Window, Candidate("Trabaja en Globex"), Context, CancellationToken.None);
    ...
    public async Task TheStateIsTheWindowPlusTheCandidate()
    {
        ...
        await Judge(judge).VerifyAsync(Window, Candidate("Trabaja en Globex"), Context, CancellationToken.None);
    ...
    public async Task Disabled_AsksNothingAndStores()
    {
        ...
        var verdict = await sut.VerifyAsync(Window, Candidate("anything"), Context, CancellationToken.None);
